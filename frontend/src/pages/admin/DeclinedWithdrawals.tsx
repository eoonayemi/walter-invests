...
const DeclinedWithdrawals = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchDWithdrawalsQuery") || ""
  );
  ...
  const {
    ...
    () => apiClient.getWithdrawals({ currentPage, status: "Declined" }),
    {
      onSuccess: (data) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchDWithdrawalsQuery", searchQuery);
  };
    ...
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">Declined Withdrawals</span>
        </div>
  ...
  if (!data && withdrawals.length === 0) {
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        ...
        <div className="flex justify-center items-center font-semibold text-lg h-full">
          <span>No Declined Withdrawals</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredWithdrawals([]);
              localStorage.removeItem("searchDWithdrawalsQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
