...
const ActiveUsers = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchActiveUsersQuery") || ""
  );
  ...
  const {
    ...
    "getActiveUsers",
    () => apiClient.getUsers({ currentPage, status: "active" }),
    {
      onSuccess: (data) => {
  ...
  const { refetch: refetchSearch, isLoading: sIsLoading } = useQuery(
    "searchActiveUsers",
    () => apiClient.searchUsers({ searchQuery, currentPage, status: "active" }),
    {
      enabled: false,
      onSuccess: (data) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchActiveUsersQuery", searchQuery);
  };
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredUsers([]);
              localStorage.removeItem("searchActiveUsersQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
