...
const AllDeposits = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchDepositsQuery") || ""
  );
  ...
  const {
    ...
    "getAllDeposits",
    () => apiClient.getDeposits({ currentPage, status: "All" }),
    {
      onSuccess: (data) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchDepositsQuery", searchQuery);
  };
    ...
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">All Deposits</span>
        </div>
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredDeposits([]);
              localStorage.removeItem("searchDepositsQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
