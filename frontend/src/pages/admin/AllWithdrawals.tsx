...
const AllWithdrawals = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchWithdrawalsQuery") || ""
  );
  ...
  const {
    ...
    "getAllWithdrawals",
    () => apiClient.getWithdrawals({ currentPage, status: "All" }),
    {
      onSuccess: (data) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchWithdrawalsQuery", searchQuery);
  };
    ...
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">All Withdrawals</span>
        </div>
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredWithdrawals([]);
              localStorage.removeItem("searchWithdrawalsQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
