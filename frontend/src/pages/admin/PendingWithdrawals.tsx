...
const PendingWithdrawals = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchPWithdrawalsQuery") || ""
  );
  ...
  const {
    ...
    () => apiClient.getWithdrawals({ currentPage, status: "Pending" }),
    {
      onSuccess: (data) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchPWithdrawalsQuery", searchQuery);
  };
    ...
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">Pending Withdrawals</span>
        </div>
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredWithdrawals([]);
              localStorage.removeItem("searchPWithdrawalsQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
        ...
        <table className="border-collapse text-sm overflow-hidden w-full">
          <thead className="bg-my-blue">
            <tr className="bg-my-blue text-white">
              ...
              <th className="text-center py-4 px-6 font-semibold">User Bank</th>
          ...
          <tbody className="bg-white">
            {withdrawals.map((withdrawal: TransactionType, i: number) => (
              <tr
                key={i}
                className={`${
                  i < data.withdrawals.length - 1 && "border-b border-slate-200"
                } py-4 text-slate-600`}
              >
                <td className="text-center py-4 px-6 capitalize">
                  {withdrawal.userName}
                </td>
                <td className="text-center py-4 px-6">{withdrawal.amount}</td>
                <td className="text-center py-4 px-6">{withdrawal.charge}</td>
                <td className="text-center py-4 px-6">
                  {withdrawal.returnAmt}
                </td>
                <td className="text-center py-4 px-6">
                  {withdrawal.bankName} <hr />
                  {withdrawal.accountName} <hr />
                  ...
                  {withdrawal.status === "Pending" ? (
                    <>
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleTransact("decline", withdrawal._id)
                        }
                        disabled={isProcessing}
                      >
                        <IconContext.Provider
