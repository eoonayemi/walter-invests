...
const DeclinedDeposits = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchDDepositsQuery") || ""
  );
  ...
  const {
    ...
    "getDeclinedDeposits",
    () => apiClient.getDeposits({ currentPage, status: "Declined" }),
    {
      onSuccess: (data) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchDDepositsQuery", searchQuery);
  };
    ...
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">Declined Deposits</span>
        </div>
  ...
  if (!data && deposits.length === 0) {
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        ...
        <div className="flex justify-center items-center font-semibold text-lg h-full">
          <span>No Declined Deposits</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredDeposits([]);
              localStorage.removeItem("searchDDepositsQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
