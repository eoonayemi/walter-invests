...
const PendingDeposits = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(
    localStorage.getItem("searchPDepositsQuery") || ""
  );
  ...
  const {
    ...
    "getPendingDeposits",
    () => apiClient.getDeposits({ currentPage, status: "Pending" }),
    {
      onSuccess: (data) => {
  ...
  const handleTransact = ({ status, id }: {status: string, id: string}) => {
  ...
  const handleSearch = () => {
    refetchSearch();
    localStorage.setItem("searchPDepositsQuery", searchQuery);
  };
  if (isLoading) {
    return (
      <div className="mx-4 flex flex-col gap-5 flex-1">
        <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">Pending Deposits</span>
        </div>
      ...
      <div className="flex self-end">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "") {
              setFilteredDeposits([]);
              localStorage.removeItem("searchPDepositsQuery");
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
      ...
      <div className="overflow-x-auto shadow-my-shadow">
        <table className="border-collapse text-sm overflow-hidden w-full">
          ...
          <tbody className="bg-white">
            {data?.deposits.map((deposit: TransactionType, i: number) => (
              <tr
                key={i}
                className={`${
                  i < data.deposits.length - 1 && "border-b border-slate-200"
                } py-4 text-slate-600`}
              >
                  ...
                  {deposit.status === "Pending" ? (
                    <>
                      <button
                        ...
                        onClick={() => {
                          handleTransact({ status: "approve", id: deposit._id});
                      ...
                      <button
                        ...
                        onClick={() => handleTransact({ status: "decline", id: deposit._id})}
                        disabled={isProcessing}
                      >
                        <IconContext.Provider
                          value={{ className: "font-bold text-white" }}
